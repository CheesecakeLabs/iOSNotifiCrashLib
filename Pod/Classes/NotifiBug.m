...
{
    [NotifiBug setupConfiguration:serialNumber];
    [NotifiBug saveThirdPartyHandler];
    [NotifiBug installUncaughtExceptionHandler];
}

/*
 * Checks if there is some other uncaught exception handler set previously
 * and stores it in a variable, so that it can run after NotifiBug's handler.
 */
...
{
    [[NotifiBug configuration] setHost:[self decodeUrl:NBEncodedUrl]];
    [[NotifiBug configuration] setSerialNumber:serialNumber];
}
...
{
    if ([NotifiBug reachMaximum]) {
        return;
    }

    NSArray *stack = [exception callStackSymbols];
    [[NotifiBug crash] setStackTrace:[stack componentsJoinedByString:@"\n"]];

    [NotifiBug handlerError:nil name:[exception name] reason:[exception reason]];
}
...
static void signalHandler(int signal)
{
    ...
    [NotifiBug handlerError:nil name:UncaughtExceptionHandlerSignalExceptionName reason:reason];
}
...
{
    [[[NotifiBug alloc] init] performSelectorOnMainThread:@selector(handleException:)
...
- (void)attemptSendingCrash
{
    ...
    NSData *postData = [NSJSONSerialization dataWithJSONObject:[self requestData:[NotifiBug crash]] options:0 error:nil];

    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:[[NotifiBug configuration] host]]];
...
- (void)captureCrashDataFromException:(NSException *)exception
{
    ...
    [[NotifiBug crash] setName:exception.name];
    [[NotifiBug crash] setReason:exception.reason];
    [[NotifiBug crash] setTime:[self timeStamp]];
    [[NotifiBug crash] setAppVersion:[self appVersionData]];
    [[NotifiBug crash] setDeviceModel:deviceModel()];
    [[NotifiBug crash] setOsVersion:[self osVersionData]];
}
...
- (NSMutableDictionary *)requestData:(Crash *)crash
{
    ...
    postJSONData[APP_SERIAL_NUMBER] = [[NotifiBug configuration] serialNumber];
